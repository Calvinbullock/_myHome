...
vim.api.nvim_create_autocmd({ 'BufLeave', 'CursorHold' }, {
    ...
    callback = function(ctx)
        -- Check if the current buffer's filetype is 'lua', if yes: return early
      if vim.bo[ctx.buf].filetype == 'lua' then
            return
        end
        ...
        if
            ...
            and vim.uv.fs_stat(vim.api.nvim_buf_get_name(ctx.buf)) -- Check if the buffer has an existing file backing it.
        then
            vim.cmd.update()
            vim.defer_fn(function() vim.api.nvim_echo({ { 'Autosaved!' } }, false, {}) end, 2000)
